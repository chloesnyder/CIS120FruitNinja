...
public class GameObj {
    ...
    /**
     * gravity converted to pixels per second. One "second" is actually 20
     * milliseconds, and one pixel is one meter. So 9.8 m/s^2  is actually
     * 9.8/(20 * 20).
     */
    public double g = (9.8 / (20 * 20));
    public double t = 0;
     ...
     * Constructor
     */
    public GameObj(double v_x, double v_y, int pos_x, int pos_y, int width,
    ...
    public void updateVelocity(double vi) {
        //updates velocity if there's gravity
        if (!court.zeroGravityMode) {
            t += .2;
            v_y = vi + (g * t);
        }
    }
     ...
     */
    public void move() {
        //If gravity is on, use parabolic motion
        if (!court.zeroGravityMode) {
            //t += .2;
            pos_x += (v_x * t);
            pos_y = (int) (pos_y + (v_y * t) * (g * t * t));
        } else {
            //otherwise, ZERO GRAVITY FUN!
            pos_x += v_x;
            pos_y += v_y;
        }
        //clip();
...
//            pos_y = max_y;
    ...
    public boolean intersects(GameObj obj) {
        System.out.println(obj + " " + obj.pos_x + " " + obj.pos_y);
